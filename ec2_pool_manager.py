...
from rdp_controller import (
    RDPController, RDPConnectionConfig, RDPConnectionPool,
    RDPState, create_rdp_config, test_rdp_connectivity
)
...
class UserSession:
    ...
    rdp_ready: bool = False
    rdp_controller: Optional[RDPController] = None
...
class EC2PoolManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.rdp_pool = RDPConnectionPool(max_connections=config.get('max_rdp_connections', 20))
        self.rdp_config = {
            'default_port': config.get('rdp_port', 3389),
            'default_username': config.get('rdp_username', 'Administrator'),
            'target_fps': config.get('rdp_target_fps', 20),
            'color_depth': config.get('rdp_color_depth', 32),
            'resolution': config.get('rdp_resolution', (1920, 1080)),
            'compression': config.get('rdp_compression', True),
            'connect_timeout': config.get('rdp_connect_timeout', 30)
        }
        ...
        self._rdp_monitoring_task: Optional[asyncio.Task] = None
        
        self.logger.info("EC2 Pool Manager with RDP integration initialized")
    ...
    async def start_pool_management(self) -> None:
        ...
        self._rdp_monitoring_task = asyncio.create_task(self._rdp_monitoring_loop())
    ...
    async def stop_pool_management(self) -> None:
        ...
        tasks = [self._scaling_task, self._monitoring_task, self._cleanup_task, self._rdp_monitoring_task]
        for task in tasks:
            if task:
                task.cancel()
                try:
        ...
        # Shutdown RDP connection pool
        await self.rdp_pool.shutdown_pool()
    ...
    async def request_instance(self, user_id: str, resource_spec: EC2ResourceSpec,
        ...
        try:
            ...
            # Initialize RDP connection for the instance
            await self._setup_rdp_connection(user_session, instance)
    ...
    # RDP Integration Methods
    
    async def _setup_rdp_connection(self, user_session: UserSession, instance: WindowsInstance) -> None:
        """
        Setup RDP connection for a user session.
        
        Args:
            user_session: User session to setup RDP for
            instance: Windows instance to connect to
        """
        try:
            # Get instance public IP for RDP connection
            rdp_host = instance.public_ip or instance.private_ip
            if not rdp_host:
                self.logger.error(f"No IP address available for RDP connection: {instance.instance_id}")
                return
            
            # Update user session with RDP details
            user_session.rdp_host = rdp_host
            user_session.rdp_port = self.rdp_config['default_port']
            user_session.rdp_username = instance.rdp_username
            user_session.rdp_password = instance.rdp_password  # Retrieved from instance
            
            # Wait for RDP service to be ready on the instance
            rdp_ready = await self._wait_for_rdp_ready(rdp_host, user_session.rdp_port, timeout_seconds=300)
            user_session.rdp_ready = rdp_ready
            
            if rdp_ready:
                self.logger.info(f"RDP ready for instance {instance.instance_id} at {rdp_host}:{user_session.rdp_port}")
            else:
                self.logger.warning(f"RDP not ready for instance {instance.instance_id}")
                
        except Exception as e:
            self.logger.error(f"Failed to setup RDP connection: {e}")
    
    async def _wait_for_rdp_ready(self, host: str, port: int, timeout_seconds: int = 300) -> bool:
        """
        Wait for RDP service to be ready on the target host.
        ...
        Returns:
            bool: True if RDP service is ready
        """
        start_time = time.time()
        ...
        while time.time() - start_time < timeout_seconds:
            try:
                ...
                if result == 0:
                    # Port is open, test RDP connectivity
                    rdp_ready = await test_rdp_connectivity(host, port)
                    if rdp_ready:
                        return True
                ...
                self.logger.debug(f"RDP connectivity check failed: {e}")
                await asyncio.sleep(10)
        
        return False
    
    def _generate_rdp_password(self, user_id: str) -> str:
        """
        Generate RDP password for user session.
        
        Args:
            user_id: User identifier
            
        Returns:
            str: Generated RDP password
        """
        # Generate a secure random password for the user
        import secrets
        import string
        
        # RDP requires more complex passwords
        alphabet = string.ascii_letters + string.digits + "!@#$%"
        password = ''.join(secrets.choice(alphabet) for _ in range(16))
        
        # Ensure password meets Windows complexity requirements
        return f"RDP{password}!"
    
    async def get_rdp_connection(self, user_id: str, session_id: str) -> Optional[RDPController]:
        """
        Get RDP connection for user session.
        ...
        Returns:
            RDPController: RDP controller ready for use
        """
        ...
        if not user_session.rdp_ready:
            self.logger.error(f"RDP not ready for session: {session_id}")
            return None
        
        # Check if controller already exists and is connected
        if user_session.rdp_controller and user_session.rdp_controller.is_connected():
            return user_session.rdp_controller
        
        try:
            # Create RDP configuration
            rdp_config = create_rdp_config(
                host=user_session.rdp_host,
                port=user_session.rdp_port,
                username=user_session.rdp_username,
                password=user_session.rdp_password,
                target_fps=self.rdp_config['target_fps'],
                color_depth=self.rdp_config['color_depth'],
                resolution=self.rdp_config['resolution'],
                compression=self.rdp_config['compression'],
                connect_timeout=self.rdp_config['connect_timeout']
            )
            
            # Get connection from pool
            controller = await self.rdp_pool.get_connection(rdp_config)
            if controller:
                user_session.rdp_controller = controller
                user_session.last_activity = datetime.utcnow()
                self.logger.info(f"RDP connection established for user {user_id}")
                return controller
            
        except Exception as e:
            self.logger.error(f"Failed to get RDP connection for user {user_id}: {e}")
        
        return None
    
    async def release_rdp_connection(self, user_id: str, session_id: str) -> None:
        """
        Release RDP connection for user session.
        ...
        if user_session.rdp_controller:
            connection_key = f"{user_session.rdp_host}:{user_session.rdp_port}"
            await self.rdp_pool.release_connection(connection_key)
            user_session.rdp_controller = None
            
            self.logger.info(f"Released RDP connection for user {user_id}")
    
    async def _rdp_monitoring_loop(self) -> None:
        """Background task for monitoring RDP connections."""
        while True:
            try:
                await self._monitor_rdp_connections()
                await self.rdp_pool.cleanup_idle_connections(idle_threshold_minutes=15)
                await asyncio.sleep(120)  # Check every 2 minutes
                
            except asyncio.CancelledError:
                break
            except Exception as e:
                self.logger.error(f"Error in RDP monitoring loop: {e}")
                await asyncio.sleep(120)
    
    async def _monitor_rdp_connections(self) -> None:
        """Monitor and maintain RDP connections."""
        try:
            for session_id, user_session in self.user_sessions.items():
                if user_session.rdp_ready and user_session.rdp_controller:
                    # Check if RDP connection is still alive
                    if not user_session.rdp_controller.is_connected():
                        self.logger.warning(f"RDP connection lost for session {session_id}")
                        
                        # Try to reconnect
                        try:
                            await user_session.rdp_controller.connect(retry_attempts=1)
                        except Exception as e:
                            self.logger.error(f"RDP reconnection failed: {e}")
                
        except Exception as e:
            self.logger.error(f"RDP connection monitoring failed: {e}")
    
    def get_rdp_pool_status(self) -> Dict[str, Any]:
        """Get RDP connection pool status."""
        return self.rdp_pool.get_pool_status()
    
    async def test_instance_rdp(self, instance_id: str) -> Dict[str, Any]:
        """
        Test RDP connectivity for a specific instance.
        
        Args:
            instance_id: Instance to test RDP connectivity
            
        Returns:
            dict: RDP test results
        """
        ...
        if not user_session or not user_session.rdp_ready:
            return {
                "instance_id": instance_id,
                "rdp_ready": False,
                "error": "Instance not found or RDP not ready"
            }
        
        try:
            # Create temporary RDP controller for testing
            rdp_config = create_rdp_config(
                host=user_session.rdp_host,
                port=user_session.rdp_port,
                username=user_session.rdp_username,
            ...
            controller = RDPController(rdp_config)
            test_results = await controller.test_connection()
            
            return {
                "instance_id": instance_id,
                "rdp_host": user_session.rdp_host,
                "rdp_port": user_session.rdp_port,
                "rdp_ready": user_session.rdp_ready,
                **test_results
            }
